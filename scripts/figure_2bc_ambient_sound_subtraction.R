...
if (SAVE == TRUE)
{
  p.bkg = sqrt(apply(P_bkg, 1, median))
  f = freq_bin
  p.bkg = cbind(f, p.bkg)
  p.exp = cbind(f, sqrt(P))
  save(filename, habitat, recorder, p.bkg, p.exp, L0.exp, TEMP, RH, PS0, A0,
       file=paste(dataFilename, '_',CHANNEL,'_attenuationModel','.Rdata', sep=""))
}
